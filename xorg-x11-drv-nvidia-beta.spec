...
Source5:       nvidia-beta-init
Source10:      nvidia-beta-config-display
Source11:      nvidia-beta-README.Fedora
...
Requires:        nvidia-beta-kmod >= %{version}
Requires(post):  nvidia-beta-kmod >= %{version}
...
Provides:      nvidia-beta-kmod-common = %{version}
...
Obsoletes:     nvidia-beta-kmod < %{version}
...
# Install initscript
install -D -p -m 0755 %{SOURCE5} $RPM_BUILD_ROOT%{_initrddir}/nvidia-beta
...
if [ "$1" -eq "1" ]; then
  ...
  %{_sbindir}/nvidia-beta-config-display enable &>/dev/null ||:
  # Add init script and start it
  /sbin/chkconfig --add nvidia-beta ||:
  /etc/init.d/nvidia-beta start &>/dev/null ||:
fi
...
if [ "$1" -eq "0" ]; then
    ...
    test -f %{_sbindir}/nvidia-beta-config-display && %{_sbindir}/nvidia-beta-config-display disable &>/dev/null ||:
    %{_initrddir}/nvidia-beta stop &> /dev/null ||:
    /sbin/chkconfig --del nvidia-beta ||:
fi ||:
